...
## Precision Farming / tramlines (FS25_AutoTramlineOffForMissions)

- **Where base + PF gameplay Lua lives:** it is NOT on disk in readable form. Base scripts are packed/encrypted in `<Game>.app/Contents/Resources/dataS.gar` (~3.4 GB); the Precision Farming "DLC" ships as a mod zip with assets only (0 `.lua` — its scripts are in `dataS.gar` too). The SDK folder has only engine bindings + map templates. To learn base/PF APIs, read **readable script mods** (BayernGamers' `FS25_adjustTramLinesForMissions`, `FS25_BetterContracts`, `FS25_FarmlandOverview`, `FS25_FarmlandMarket`) or the community mirror **github `Dukefarming/FS25-lua-scripting`** (branch `master`, raw files only — file-finder/tree/api are JS-rendered/blocked, guess raw paths).
- **Tramlines ("Fahrgassen") are a persistent farmland density-map property**, keyed by `farmlandId`, NOT owned by a mission. PF paints them on NPC fields; the gate is `FS25_precisionFarming.g_precisionFarming.tramlineMap.npcFieldFruitTypes[fruitTypeIndex]` (nil/false ⇒ no NPC tramlines for that crop). Emptying that table disables NPC-tramline generation at the source; the player's own seeder tramlines don't use it and stay.
- **Reset a farmland's tramlines (refill crop):** server-side, enqueue `FS25_precisionFarming.TramlineMapDensityMapTask.new():setData(farmlandId):enqueue(true)`, then `field:getFieldState():createFieldUpdateTask()` + `:setArea(field:getDensityMapPolygon())` + `g_fieldManager:addFieldUpdateTask(task)`. `g_fieldManager:getFieldById(farmlandId)` maps a farmland id to its field.
- **`loadMap` is too early for field/farmland foliage ops.** Base classes exist (fine for installing hooks / class monkeypatches, before mission generation), but field states aren't ready → per-field resets there silently no-op. Do the heavy field work from a console command run during play (or a deferred/frame-counted call).
- **Foliage changes need SAVE + full RESTART to become visible.** After a runtime density-map change, a plain "menu → reload savegame" does NOT show the refill; save and fully restart FS.
- **Farmland ownership:** iterate `g_farmlandManager.farmlands`; `farmland.id`, `farmland.farmId`; unowned/NPC = `FarmlandManager.NO_OWNER_FARM_ID`, player = `g_currentMission:getFarmId()`, spectator = `FarmManager.SPECTATOR_FARM_ID`. NPC fields display a fictional owner name but are NO_OWNER.
- **Mission hooks:** `AbstractMission:finishedPreparing()` = real server-side post-prepare hook (append). `mission:getMissionTypeName()`, `mission.field.farmland.id`, `mission:getFieldCompletion()`. Field-mission types with tramlines: `FertilizeMission` / `HerbicideMission` / `HoeMission` / `WeedMission` (`.NAME`).
- **Perf note:** re-running a mass per-field foliage rebuild every load causes a one-time load stutter (transient, not a per-frame loop). Keep mass migrations as a one-shot console command, not a loadMap step.
